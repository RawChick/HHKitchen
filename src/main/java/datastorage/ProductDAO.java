 ...
 * database that have to do with the products.
 * 
...
public class ProductDAO {
            ...
            if (resultset != null) {
                try {
                    while (resultset.next()) {
                        ...
                        products.add(new Product(productNrFromDb,
                                productNameFromDb, priceFromDb, timeFromDb));
                    }
    ...
    public boolean removeProduct(int productNr) {
        ...
        if (connection.openConnection()) {
            ...
            result = connection
                    .executeSQLDeleteStatement("DELETE FROM dish_menu_item WHERE ID = "
                            + productNr + ";");
     ...
     * @param prepTime
     *            The preparation time of a product.
     * @return True or false depending on the success of the method.
     */
    public boolean updateProduct(int productNr, String name, long price,
            long prepTime) {
        boolean result = false;
        ...
        if (connection.openConnection()) {
            result = connection
                    .executeSQLUpdateStatement("UPDATE dish_menu_item SET "
     ...
     * @return True or false depending on the success of the method.
     */
    public int createNewProduct(long price, int menuID, String name,
            String description, long prepTime) {
        int productNr = 0;

        DatabaseConnection connection = new DatabaseConnection();

        if (connection.openConnection()) {
            productNr = connection
                    .executeSQLInsertStatement(
                            "INSERT INTO `dish_menu_item`(`price`, `menu_ID`, `name`, `description`, `cooking_minutes`) VALUES ("
                                    + price + ", "
                                    + menuID + ", '"
                                    + name + "', '"
                                    + description + "', "
