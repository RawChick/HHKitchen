 ...
 * database that have to do with the products from an order.
 * 
...
public class OrderRowDAO {
     ...
     */
    public List<OrderRow> retrieveOrderRows(int orderNr) {
        List<OrderRow> retrievedOrderRows = new ArrayList<OrderRow>();
            ...
            if (resultset != null) {
                try {
                    while (resultset.next()) {
                        ...
                        retrievedOrderRows.add(new OrderRow(orderNr,
                                dishItemIDFromDb, amountFromDb));
                    }
