 ...
 * database that have to do with the ingredients of a product.
 * 
...
public class ProductIngredientsDAO {
    ...
    public List<ProductIngredients> retrieveIngredients(int productNr) {
        ...
        if (connection.openConnection()) {
            ...
            ResultSet resultset = connection
                    .executeSQLSelectStatement("SELECT " + "food.name, "
                            + "food.id, " + "ingredient.food_ID, "
            ...
            if (resultset != null) {
                try {
                    while (resultset.next()) {
                        ...
                        ProductIngredients newIngredient = new ProductIngredients(
                                ingredientNrFromDb, ingredientNameFromDb,
                                productNr, quantityFromDb, unitFromDb);
    ...
    public void updateIngredientSpecs(int productNr, int ingredientNr, String name, int quantity) {
        ...
        if (connection.openConnection()) {
            try {
                connection
                        .executeSQLUpdateStatement("UPDATE ingredient SET quantity = "
                                + quantity + " WHERE food_ID = " + productNr);
            } catch (Exception ex) {
                ex.printStackTrace();
            }

            try {
                connection.executeSQLUpdateStatement("UPDATE food SET name = '"
                        + name + "' WHERE ID = " + ingredientNr);
            } catch (Exception ex) {
                ex.printStackTrace();
            }
     ...
     */
    public void insertProductIngredients(int ingredientNr, int productNr,
            long quantity, String unit) {
        DatabaseConnection connection = new DatabaseConnection();

        if (connection.openConnection()) {
            connection
                    .executeSQLInsertStatement("INSERT INTO ingredient (`food_ID`, `menu_item_ID`, `quantity`, `unit`) VALUES ("
                            + ingredientNr
                            + ", "
                            + productNr
                            + ", "
