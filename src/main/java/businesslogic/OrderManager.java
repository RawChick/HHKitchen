...
/**
 * This class delegates persistence to the datastorage layer and receives
 * entity's from the datastorage layer.
 * 
 * 
...
public class OrderManager {
    ...
    private List<Order> orders, newOrders, cancelledOrders;
    ...
    /**
     * This method calls a method from the OrderDAO to retrieve all new orders
     * from the database.
     */
    ...
    /**
     * This method calls a method from the OrderDAO to retrieve all cancelled
    ...
    public List<ProductIngredients> retrieveIngredients(int productNr) {
        List<ProductIngredients> rowIngredients = new ArrayList<ProductIngredients>();
        ...
        if (foundProductIngredients == false) {
            rowIngredients = productIngredientsDAO
    ...
    /**
     * This method calls a method from the ProductDAO to retrieve all products
     * from the database.
     */
    ...
    /**
     * This method calls a method from the IngredientsDAO to retrieve all
     ...
     * 
     * @param product
     *            Contains all attributes of a product.
     */
     ...
     */
    public List<OrderRow> getProducts(int orderNr) {
        List<OrderRow> orderRowsProducts = new ArrayList<OrderRow>();
        ...
        if (result == false) {
            List<OrderRow> retrievedOrderRows = orderRowDAO
                    .retrieveOrderRows(orderNr);
    ...
    /**
     * This method calls a method from the EmployeeDAO to find an entered
     * employeeNumber in the database.
     * 
    ...
    /**
     * This method calls a method from the ProductDAO to update a product in the
     * database.
     * 
     * @param productNr
     ...
     * @return True or false depending on the success of the method.
     */
    public boolean updateProduct(int productNr, String name, long prepTime,
            long price) {
        boolean result = productDAO.updateProduct(productNr, name, price,
                prepTime);
    ...
    /**
     * This method calls a method from the ProductDAO to create a new product in
     * the database.
     * 
     * @param priceValue
     *            The price of the product.
     ...
     * @param prepTimeValue
     *            The preperation time of the product.
     * @return A new product number.
     */
    public int createNewProduct(long priceValue, int menuID, String nameValue,
            String descriptionValue, long prepTimeValue) {
        int newProductNr = productDAO.createNewProduct(priceValue, menuID,
                nameValue, descriptionValue, prepTimeValue);

        products.add(new Product(newProductNr, nameValue, priceValue,
                prepTimeValue));
    ...
    /**
     * This method calls a method from the ProductDAO to delete a product from
     * the database.
     * 
     * @param productNr
     *            A number to indicate which product is meant.
     * @return True or false depending on the success of the method.
     */
    ...
    public void updateIngredientSpecs(JTable table, int row, int productNr) {
        int ingredientNr = Integer
                .parseInt(table.getValueAt(row, 0).toString());
        String name = (String) table.getValueAt(row, 1);
        int quantity = Integer.parseInt(table.getValueAt(row, 2).toString());

        productIngredientsDAO.updateIngredientSpecs(productNr, ingredientNr,
                name, quantity);
     ...
     * @param ingredient
     */
    public void insertProductIngredients(List<ArrayList> ingredient) {
        int ingredientNr = 0;
        int productNr = 0;
        long quantity = 0;
        String unit = null;
        ...
        for (ArrayList AL : ingredient) {
            ...
            productIngredientsDAO.insertProductIngredients(ingredientNr,
                    productNr, quantity, unit);
        }
    }
}
